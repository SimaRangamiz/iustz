    ...
    bool Attack(char command, int index)
{
    ...
    if (command == 'A') {
        attackerPower = model->getEnemy().getPower() + model->getEnemy().getWeapon()[index].getPower();
        defenderPower = model->getHuman().getPower();
    } else {
        attackerPower = model->getHuman().getWarmskill() + model->getHuman().getColdskill() + model->getHuman().getWeapon()[index].getPower();
        defenderPower = model->getEnemy().getPower();
    }
    if (command == 'A' && model->getEnemy().getStamina() < attackerPower) {
        cerr << "\nLow Enemy Stamina! \nTry Again. \n";
        return false;
    } else if (command == 'B' && model->getHuman().getStamina() < attackerPower) {
        cerr << "\nLow Human Stamina! \nTry Again. \n";
        return false;
    }

    if (command == 'A' && model->getHuman().getArmorItems()[index].isArmorItem()) {
        return true; 
    }

    int newDefenderHP = model->getEnemy().getHP() - attackerPower;
    model->getEnemy().setHP(newDefenderHP);

    if (command == 'A') {
        int newEnemyStamina = model->getEnemy().getStamina() - attackerPower;
        model->getEnemy().setStamina(newEnemyStamina);
    } else {
        int newHumanStamina = model->getHuman().getStamina() - attackerPower;
        model->getHuman().setStamina(newHumanStamina);
    }

    if (model->getEnemy().getHP() <= 0) {
        cout << "\nThe enemy was defeated!\n";
        return true;
    }

    if (command == 'A' && model->getEnemy().getWeapon()[index].getModel() == 't') {
        model->getEnemy().getWeapon()[index].addCount(-1);
    } else if (command == 'B' && model->getHuman().getWeapon()[index].getModel() == 't') {
        model->getHuman().getWeapon()[index].addCount(-1);
    }

    if (command == 'A' && !model->getHuman().isAlive()) {
        cout << "\nGame Over! :(\n";
        exit(0);
    } else if (command == 'B' && !model->getEnemy().isAlive()) {
    }
...
int main()
{
    ...
    Store store(Stamina, HP, weapons, model);
     ...
     srand(time(NULL)); // to get a random number generator
